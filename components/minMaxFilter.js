...
// Copy of CBB version — spinner arrows kept visible (no appearance hiding)
...
export function createMinMaxFilter(cell, onRendered, success, cancel, editorParams = {}) {
    ...
    container.style.cssText = `display:flex;flex-direction:column;gap:2px;width:100%;max-width:${maxWidth}px;margin:0 auto;`;
    
    const inputStyle = `width:100%;padding:2px 3px;font-size:9px;border:1px solid #ccc;border-radius:2px;text-align:center;box-sizing:border-box;`;
    ...
    function applyFilter() {
        ...
        filterTimeout = setTimeout(() => {
            ...
            if (minVal === null && maxVal === null) { success(''); } else { success({ min: minVal, max: maxVal }); }
        }, 300);
    }
