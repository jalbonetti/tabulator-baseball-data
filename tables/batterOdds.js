...
export class BatterOddsTable extends BaseTable {
    constructor(elementId) {
        ...
        this.propAbbrevMap = { 'Hits + Runs + RBIs': 'H+R+RBI', 'Total Bases': 'TB', 'Strikeouts': 'K', 'Walks': 'BB', 'Stolen Bases': 'SB', 'Singles': '1B', 'Doubles': '2B', 'Home Runs': 'HR' };
    }
    ...
    getColumns(isSmallScreen = false) {
        ...
        const kellyFormatter = (cell) => { const v = cell.getValue(); if (v == null || v === '' || v === '-') return '-'; const n = parseFloat(v); if (isNaN(n)) return '-'; const b = getBankrollValue('Batter Quarter Kelly %'); if (b > 0) return '$' + (n * b).toFixed(2); return (n * 100).toFixed(1) + '%'; };
    ...
    scanDataForMaxWidths(data) {
        ...
        data.forEach(row => { Object.keys(maxWidths).forEach(field => { const value = row[field]; if (value != null && value !== '') { let dv = String(value); if (field === 'Batter Prop Type') dv = this.abbreviateProp(value); if (field === 'Batter Matchup') dv = this.abbreviateMatchup(value); if (field.includes('Odds') && field !== 'Batter Best Odds Books') { const n = parseInt(value, 10); if (!isNaN(n)) dv = n > 0 ? `+${n}` : `${n}`; } if (field === 'EV %' || field === 'Quarter Kelly %') { const n = parseFloat(value); if (!isNaN(n)) dv = (n*100).toFixed(1)+'%'; } if (field === 'Link') dv = 'Bet'; const w = ctx.measureText(dv).width; if (w > maxWidths[field]) maxWidths[field] = w; } }); });
